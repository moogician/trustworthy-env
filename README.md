```
  ...
  # Run benchmark audit pipeline (policy + static verification + runtime verification)
  python3 detect.py audit kernelbench-demo --benchmark-id kernelbench --out audit-report.json

  # Include runtime verification by executing benchmark command
  python3 detect.py audit kernelbench-demo --benchmark-id kernelbench --run-cmd "python3 scripts/verify_bench.py" --timeout-s 90
```
...
See `audit-system-mvp.md` for the architecture and report format of the new audit pipeline.
